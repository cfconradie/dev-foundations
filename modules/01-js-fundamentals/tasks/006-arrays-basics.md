...
**Step 1 — build it yourself, no AI:** build an array of 5 objects (`{name, price}`), then without
using higher-order array methods (no `.map`/`.filter` yet — use loops), find the total price, find
the most expensive item, and build a new array of just the names.
