...
**Step 1 — build it yourself, no AI:** split your project's pricing logic into real files that
will keep growing as the course continues: a `pricing.js` with named exports (`calculateTotal`,
the `Map`-based `findRuleById` from task 010), a `format.js` with a default export
(`formatCurrency`, pulling from task 021's currency formatting), and a `main.js` that imports and
uses both against a small set of real `PricingRule` data. Before splitting, write down (one
sentence each) *why* you're putting each function in the file you chose — not just "it fits" —
since that's exactly the reasoning task 019's mini-project review will probe.
